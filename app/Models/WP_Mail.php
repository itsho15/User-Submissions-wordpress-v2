...
Class WP_Mail{
		...
		if(is_array($path)){
			...
			foreach($path as $path_) {
				if(!file_exists($path_)){
					return "Attachment not found at".$path;
				}else{
	...
	public function send(){
		if(count($this->to) === 0){
			return 'You must set at least 1 recipient';
		}

		if(empty($this->template)){
		   return 'You must set a template';
		}

		if($this->sendAsHTML){
			add_filter('wp_mail_content_type', array($this, 'HTMLFilter'));
		}
			dd( wp_mail($this->to, $this->buildSubject(), $this->render(), $this->buildHeaders(), $this->attachments));
