...
<div id="edit_{{ $submission->id }}" class="modal fade " role="dialog">
    ...
    <div class="modal-dialog modal-lg">
        <form  method="POST" id="updatesub_{{ $submission->type  }}" action="<?php echo esc_url(admin_url("submmtion/update/") . $submission->id); ?>" enctype="multipart/form-data">
    <!-- Modal content-->
 ...
 <!-- delete -->
<div id="del_admin{{ $submission->id }}" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <form action="<?php echo esc_url(admin_url("submmtion/destroy/") . $submission->id); ?>" method="post" >
        <input type="hidden" name="_token" value="{{ wpLumen()->csrf() }}"/>
      <div class="modal-body">
        <h4>are you sure you want to delete this submission</h4>
      </div>
      <div class="modal-footer">
        <button type="button" class="button btn-info" data-dismiss="modal">close</button>
        <input type="submit" class="button btn-info" value="yes">
      </div>
 ...
 <!-- approve -->
<div id="approve_{{ $submission->id }}" class="modal fade " role="dialog">
      <div class="modal-dialog modal-lg">
        ...
        <form name="Submission_edit_update" id="submitForm" method="POST" action="<?php echo esc_url(admin_url("submmtion/approve/") . $submission->id); ?>" enctype="multipart/form-data">

        <input type="hidden" class="approve" name="YesOrNo" <?php if ($submission->Approval !== Null) {echo 'value="' . $submission->Approval . '" ';}?>>
            <div class="form-group" >
                <label for="training_keywords">Approve ?</label>
                <div id="radioBtn">
                    <a class="button <?php if ($submission->Approval === 1) {echo 'active';}?>" data-toggle="approve" data-title="1">Approved/Published</a>
                    <a class="button <?php if ($submission->Approval === 0) {echo 'active';}?>" data-toggle="approve" data-title="0">Disapprove</a>
                </div>

            </div>

            <div class="form-group">
                    <label for="reason_approve">if No , reason: </label>
                    <textarea class="form-control" id="reason_approve" name="reason_approve">{{ $submission->reason_approve }}</textarea>
            </div>

             <input type="hidden" name="id" value="{{ $submission->id }}">
             <button type="submit" name="approve" class="btn btn-info pull-left">Submit</button>
            </form>
